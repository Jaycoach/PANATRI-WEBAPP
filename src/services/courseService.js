...
class CourseService {
  ...
  async deleteCourse(courseId, userId) {
    try {
      ...
      if (enrolledUsers > 0) {
        throw new ErrorResponse('No se puede eliminar un curso con estudiantes matriculados', 400);
      }
  ...
  async deleteModule(courseId, moduleId, userId) {
    try {
      ...
      if (hasVideos) {
        throw new ErrorResponse('No se puede eliminar un módulo con videos asociados', 400);
      }
  ...
  async addReview(courseId, userId, rating, comment) {
    try {
      ...
      if (!user) {
        throw new ErrorResponse('Solo los estudiantes matriculados pueden dejar reseñas', 403);
      }
